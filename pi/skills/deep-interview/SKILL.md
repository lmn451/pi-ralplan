...
<Purpose>
Deep Interview implements Ouroboros-inspired Socratic questioning with mathematical ambiguity scoring. It replaces vague ideas with crystal-clear specifications by asking targeted questions that expose hidden assumptions, measuring clarity across weighted dimensions, and refusing to proceed until ambiguity drops below the resolved threshold for this run. The output feeds into a gated pipeline: **deep-interview → consensus planning → pending approval → explicitly approved execution**, ensuring maximum clarity before any mutation starts.
</Purpose>
...
- User wants to explore options or brainstorm -- use the host's planning entrypoint (e.g. `/ralplan` or `/brainstorm`) instead
- User wants a quick fix or single change -- make the change directly
...
<Why_This_Exists>
AI can build anything. The hard part is knowing what to build. A single-pass approach that expands an idea into a spec in one shot struggles with genuinely vague inputs. It asks "what do you want?" instead of "what are you assuming?" Deep Interview applies Socratic methodology to iteratively expose assumptions and mathematically gate readiness, ensuring the AI has genuine clarity before spending execution cycles.
...
- Persist interview state to `plans/.deep-interview/state.json` for resume across session interruptions
...
Complete this phase before Phase 1, before brownfield exploration, before persisting interview state, before Round 0, and before any ambiguity scoring. Do not continue if the resolved threshold and source are unknown.

1. **Resolve the threshold from the run argument**:
   - `--quick`: higher threshold (looser gate, fewer rounds) — `0.35`.
   - `--standard` (default when no mode flag is given): `0.2`.
   - `--deep`: lower threshold (stricter gate, more rounds) — `0.1`.
2. **Resolve threshold and source**:
   - Pick the threshold for whichever mode flag is present; if none is present, use `--standard`.
   - Set these run variables exactly: `<resolvedThreshold>`, `<resolvedThresholdPercent>`, and `<resolvedThresholdSource>` (the source is the mode flag that was used, e.g. `--quick`, `--standard`, `--deep`, or `default` when no flag was supplied and the standard default applied).
...
4. **Carry threshold source forward mechanically**:
   ...
   - Include `threshold_source` alongside `threshold` in the first persisted state payload at `plans/.deep-interview/state.json` and preserve it on later state updates.
...
3. **For brownfield**: Build the first-round context before designing Round 1 questions:
   ...
   - Wait until the summary exists before ambiguity scoring, weakest-dimension selection, brownfield exploration prompts, or any bridge to host consensus planning (e.g. `/ralplan`).
     3.7. **Artifact path discipline**:
   - Final specs MUST be written to `plans/deep-interview-{slug}.md` exactly.
   - Ephemeral interview artifacts (scoring scratchpads, prompt-safe summaries, transient queues, resume metadata) belong in `plans/.deep-interview/`, never in the repo root or arbitrary working files.

4. **Initialize state** by writing the following JSON to `plans/.deep-interview/state.json` with the `Write` tool:
...
Update interview state with the new round, global scores, per-component `topology.components[].clarity_scores`, `topology.components[].weakest_dimension`, ontology snapshot, and `topology.last_targeted_component_id` by rewriting `plans/.deep-interview/state.json`.
...
2. **Write to file**: `plans/deep-interview-{slug}.md`
   - Always use this exact final spec path. Do not write temporary working files to the repo root or other ad hoc paths.
   - For ephemeral artifacts during interview rounds (for example scoring intermediate results, prompt-safe summaries, question queues, or resume metadata), use `plans/.deep-interview/`.
...
After the spec is written, mark it `pending approval` and present next-step options via `AskUserQuestion`. Until the user explicitly selects an option, the deep-interview module MUST NOT run mutation-oriented shell commands, edit source files, commit, push, open PRs, invoke execution mechanisms, or delegate implementation tasks. The deliverable of this skill is the spec at `plans/deep-interview-{slug}.md`; it gathers requirements only and never auto-executes:

**Question:** "Your spec is ready (ambiguity: {score}%). How would you like to proceed?"

**Options:**

1. **Refine with consensus planning (Recommended)**
   - Description: "Hand the spec to the host's consensus planner to produce an implementation plan, then stop for explicit execution approval. Maximum quality."
   - Action: Only after the user selects this option, hand the spec at `plans/deep-interview-{slug}.md` to the host's consensus planning entrypoint. In this repo that is `/ralplan` (aliased as `/brainstorm`) — invoke it with the spec file path as context. Because the deep interview already gathered requirements, the planner should skip its own interview phase and go straight to consensus planning. When planning completes and produces a plan under `plans/`, stop with that plan marked `pending approval`; do not automatically execute.
   - Pipeline: `deep-interview spec → explicit approval to refine → host consensus planning (e.g. /ralplan) → pending approval → separate execution approval`

2. **Hand off to the host's execution mechanism**
   - Description: "Pass the spec to whatever execution path the host provides (planner, agent runner, or direct implementation). Faster but without a separate consensus-refinement pass."
   - Action: Only after the user explicitly selects this execution option, hand the spec file path to the host's available execution mechanism. The spec serves as the requirements/plan input for that step.

3. **Refine further**
   - Description: "Continue interviewing to improve clarity (current: {score}%)"
   - Action: Return to Phase 2 interview loop.

**IMPORTANT:** Do NOT implement directly. The deep-interview skill is a requirements skill, not an execution skill. On explicit selection of a next step, hand off the spec to the chosen host mechanism. If oversized initial context was summarized, pass the spec and prompt-safe summary forward, not the raw oversized source material. Without an explicit next-step selection, stop with the spec marked `pending approval`.

### Approval-Gated Refinement Path (Recommended)

```
Stage 1: Deep Interview          Stage 2: Host consensus planning   Stage 3: Separate approval
┌─────────────────────┐    ┌───────────────────────────┐    ┌──────────────────────┐
│ Socratic Q&A        │    │ Planner creates plan      │    │ User chooses if/how  │
│ Ambiguity scoring   │───>│ Reviewer/Critic validates │───>│ execution proceeds   │
│ Challenge agents    │    │ Loop until consensus      │    │ via host mechanism   │
│ Spec crystallization│    │ Plan written under plans/ │    │ no auto-handoff      │
│ Gate: ≤<resolvedThresholdPercent> ambiguity│    │                           │    │                      │
└─────────────────────┘    └───────────────────────────┘    └──────────────────────┘
Output: spec.md            Output: consensus plan           Output: pending approval
```
...
2. **Consensus planning** gates on _feasibility_ — is the approach architecturally sound?
3. **Separate approval** gates on _consent_ — does the user explicitly choose an execution path?
...
- Skip Stage 2 → execution may plan poorly (no reviewer/critic challenge)
- Skip Stage 3 → no execution (just a refined plan), by design

</Steps>

<Agent_Spawning>
This skill is **execution-agnostic**: it never hardcodes how a subagent is launched. The host (pi) may expose several subagent systems — a `Task`/subagent tool, a nested session, or direct LLM calls. Use whichever one is available.

When a step says to "spawn the `<name>` agent/persona", do this:

1. Load the persona's system prompt from `agents/<name>.md` (the file body, below its YAML frontmatter, IS the system prompt).
2. Spawn a subagent through any available host mechanism, passing that body as the subagent's system prompt, **preferably with isolated context** so the persona's work doesn't pollute the main interview thread.
3. Honor the persona's `disallowedTools` frontmatter when the host lets you scope tools.
4. Hand the subagent only the focused task (e.g. "find authentication implementation") and consume its final result.

Available personas (in `agents/`): `explore`, `analyst`, `architect`, `planner`, `critic`, `executor`. Do not invent persona names; spawn only these. If the host exposes no subagent mechanism at all, fall back to adopting the persona inline (read `agents/<name>.md` and follow it as a role prompt) and say so.
</Agent_Spawning>

<Tool_Usage>

- Use AskUserQuestion for interview questions — provides clickable UI with contextual options
- Spawn the `explore` persona (see `<Agent_Spawning>`) for brownfield codebase exploration (run BEFORE asking user about codebase)
...
- Persist interview state to `plans/.deep-interview/state.json` using the `Write`/`Read` file tools; the initial and subsequent state payloads must include `threshold_source` alongside `threshold`
- Use the `Write` tool to save the final spec to `plans/deep-interview-{slug}.md` exactly; use `plans/.deep-interview/` for ephemeral artifacts
- Hand the spec off to the host's planning/execution mechanism only after explicit approval — never implement directly
...
<Final_Checklist>
- [ ] Phase 0 completed before Phase 1: the threshold was resolved from the run argument (`--quick|--standard|--deep`, default `--standard`), and the first user-visible line was `Deep Interview threshold: <resolvedThresholdPercent> (source: <resolvedThresholdSource>)`
...
- [ ] Spec file written to `plans/deep-interview-{slug}.md` exactly; ephemeral artifacts stayed under `plans/.deep-interview/`
...
- [ ] Selected next step handed to the host's planning/execution mechanism only after explicit approval (never direct implementation)
- [ ] If 3-stage pipeline selected: host consensus planning (e.g. `/ralplan`) invoked, then stopped with the consensus plan marked `pending approval` until the user explicitly approves execution
...
The ambiguity threshold and related behavior are resolved from the run argument, not a config file:

| Setting | How it is resolved |
|---------|--------------------|
| `ambiguityThreshold` | `--quick` → `0.35`, `--standard` (default) → `0.2`, `--deep` → `0.1` |
| `maxRounds` | 20 (hard cap) |
| `softWarningRounds` | 10 |
| `minRoundsBeforeExit` | 3 |
| `scoringModel` | opus (temperature 0.1) |

## Resume

If interrupted, run the skill again. It reads state from `plans/.deep-interview/state.json` and resumes from the last completed round.

## Integration with Host Execution

A host planning or execution entrypoint that receives a vague input (no file paths, function names, or concrete anchors) can redirect to deep-interview first:

```

User: "build me a thing"
Host: "Your request is quite open-ended. Would you like to run a deep interview first to clarify requirements?"
[Yes, interview first] [No, expand directly]

```

If the user chooses interview, the host invokes the deep-interview skill. When the interview completes and the user selects a next step, the spec becomes the requirements/plan input for that step.

## Approval-Gated Pipeline: deep-interview → consensus planning → pending approval
...
→ Spec written to plans/deep-interview-{slug}.md
→ User explicitly selects "Refine with consensus planning"
→ Host consensus planner (e.g. /ralplan) receives the spec as input
→ Planner creates implementation plan from spec
→ Reviewer/Critic validates architectural soundness, quality, and testability
→ Loop until consensus
→ Consensus plan written under plans/
→ Stop with the consensus plan marked pending approval
→ Only a separate explicit execution approval may begin execution

```

**The consensus planner receives the spec as input** because the deep interview already did the requirements gathering, so the planner can skip its own interview phase and go straight to consensus planning. The consensus plan includes:
- Decision-record summary (Principles, Decision Drivers, Options)
...
**Execution is a separate approval-gated step.** This skill must not auto-execute merely because a spec or plan exists.

## Integration with the Host Planning Gate

A host pre-execution planning gate (e.g. `/ralplan`) that redirects vague prompts to planning can use deep interview as an alternative redirect target for prompts that are too vague even for that gate:

```

Vague prompt → host planning gate → deep-interview (if extremely vague) → consensus planning (with clear spec) → pending approval → explicitly approved execution
