...
- Soften auto-start heuristic: add a `hasAutoStarted` module-level flag so auto-start fires at most once per extension lifetime.
...
- In `startTimer` and command/tool focus setters, truncate focus with `truncateFocus()`.
- Add `MAX_DURATION_MINUTES = 180` constant.
- In `parseDurationMinutes`, cap each parsed value to `MAX_DURATION_MINUTES`.
...
- In `registerShortcut` handler, use `ctx?.ui.notify` for start/stop notifications.
